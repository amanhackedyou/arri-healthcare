...
const Reason = ({ icon, title }: { icon: React.ReactNode, title: string }) => {
  return (<div className={styles.reason}>
    <div className='min-w-20 min-h-20 relative z-10  text-white rounded-full'>
      <div className='min-w-full min-h-full bg-primaryBlue absolute z-10 flex items-center justify-center rounded-full'>
        <span className='!text-4xl'>{icon}</span>
      </div>

      <div className='min-w-full min-h-full bg-[#fe8f01] absolute top-0 z-0 rounded-full translate-x-[3px] -translate-y-px'></div>
